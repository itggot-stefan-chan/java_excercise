...
public class Ex9FuncDecompTicTacToe {
    ...
    void program() {
        ...
        while (winner == null) {
            out.println("Player is " + actual.name);
            out.println("Select position to put mark (0-8)");
            board = replaceBoard(sc.nextInt(), actual.mark, board);
            plotBoard(board);
            if (actual == p1) {
                actual = p2;
            } else {
    ...
    char[] replaceBoard(int i, char mark, char[] board) {
        if(board[i] == EMPTY) {
            board[i] = mark;
        }
        else if(board[i] == 'X' || board[i] == 'O') {
            out.println("You can not do that");
        }
        return board;
    }
